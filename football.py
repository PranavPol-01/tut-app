...
for i, sport in enumerate(sports):
    ...
    image = Image.open("assets\\fbpage2.png")
    # Calculate the aspect ratio
    aspect_ratio = image.width / image.height
    new_width1 = min(590, int(590 * aspect_ratio))
    new_height1 = min(590, int(590 / aspect_ratio))
    ...
    notebook.add(frame_6, text=f'Positions')
    image = Image.open("assets\\fbmedia1.png")
    # Calculate the aspect ratio
    aspect_ratio = image.width / image.height
    new_width = min(900, int(900 * aspect_ratio))
    new_height = min(900, int(900 / aspect_ratio))
    ...
    # add sport information label
    sport_info_label = ttk.Label(frame_6, padding=5,wraplength=900, image=shot, compound='right')
    sport_info_label.pack()
    ...
    frame_4 = ttk.Frame(notebook)
    notebook.add(frame_4, text=f'Scoring')
    ...
    # open the video file with OpenCV
    video = cv2.VideoCapture('assets\\3-control-skills-to-practice-first-football-soccer-shorts.mp4')
    ...
    frame_5 = ttk.Frame(notebook)
    notebook.add(frame_5, text=f'Ground Dimensions')
